...
export const SecureRoute: FunctionComponent<OwnProps> = ({
  component,
  extraArgs,
  ...rest
}: OwnProps) => {
  const token = useSelector<RootState, AuthenticationInformation | undefined>(
    (state) => state.user.authenticationInformation
  );
      ...
      render={(props) => {
        if (isNotExpired(token)) {
          if (typeof component === "function") {
            ...
            "WrappedComponent" in component &&
            typeof component["WrappedComponent"] === "function"
          ) {
            const CC = component as ConnectedComponent<
              FunctionComponent,
              typeof props
            >;
