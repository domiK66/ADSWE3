...
import { BuildForm, FieldDescriptionType, FormDescription } from '../../services/utils/FormBuilder';
...
const formDescription: (mode: 'add' | 'edit', type: 'coral' | 'animal') => FormDescription<FormDataAnimal | FormDataCoral> = (mode, type) => ({
  name: `${mode}-${type}`,
  fields: [...itemFields, ...(type === 'animal' ? animalFields : coralFields)],
  submitLabel: mode
});

// eslint-disable-next-line import/no-anonymous-default-export
export default (type: 'coral' | 'animal', mode: 'add' | 'edit'): FC<RouteComponentProps<{ id?: string }>> =>
  ({ match, history }) => {
    ...
    const submitAdd = async (itemData: Animal | Coral) => {
      ...
      const itemDispatch = type === 'animal' ? thunkDispatchAdd(addAnimalAction(itemData)) : thunkDispatchAdd(addCoralAction(itemData));
    ...
    const submitEdit = async (itemData: Animal | Coral) => {
      ...
      const itemDispatch = type === 'animal' ? thunkDispatchEdit(editAnimalAction(itemData.id ?? '', itemData)) : thunkDispatchEdit(editCoralAction(itemData.id ?? '', itemData));
      ...
      <IonPage>
        <IonHeader>
          <IonToolbar>
            ...
            <IonTitle>
              {mode.toUpperCase()} {type.toUpperCase()}
            </IonTitle>
          </IonToolbar>
        </IonHeader>
