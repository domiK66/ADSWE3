...
import { IonButton, IonToast, IonItem, IonLabel, IonInput, IonIcon, IonDatetime, IonSelectOption, IonSelect, IonSpinner } from '@ionic/react';
...
type FormActions = { type: 'form/loading'; name: string; isLoading: boolean } | { type: 'form/error'; name: string; errorMessage: string };
...
export const formBuilderReducer = createReducer<FormBuilderState, FormActions>({})
  .handleType('form/error', (state, action) => ({ ...state, [action.name]: { ...state[action.name], errorMessage: action.errorMessage } }))
...
const DumpState = ({ debug, formState }: { debug: boolean | undefined; formState: {} }) => (debug ? <p>State: {JSON.stringify(formState)}</p> : <span></span>);

const spinner = (isLoading: boolean) => (isLoading ? <IonSpinner /> : <></>);
...
export const BuildForm = <T extends { [index: string]: any }>({ fields, name, submitLabel, debug }: FormDescription<T>) => ({
  Form: ({ handleSubmit, initialState }: FormProps<T>) => {
    const [formState, { raw }] = useFormState(initialState);
    const { isLoading, errorMessage } = useSelector<FormBuilderReduxState, LoaderState>(
      state =>
        state?.formBuilder?.[name] || {
          isLoading: false,
          errorMessage: ''
        }
    );
    ...
    const showError = (name: any) =>
      formState.errors[name] ? (
        <div className="field-error-msg">
          <IonIcon icon={closeCircle} color="red" /> {formState.errors[name]}
        </div>
      ) : undefined;

    const createTextField = <T extends { [index: string]: any }>({ def, widgetState }: { def: TextFieldDescription<T>; widgetState: ReturnType<typeof raw> }) => (
      <IonItem key={def.name.toString()}>
        <IonLabel position={def.position} color={def.color}>
          {def.label}
        </IonLabel>
    ...
    const createDateField = <T extends { [index: string]: any }>({ def, widgetState }: { def: DateFieldDescription<T>; widgetState: ReturnType<typeof raw> }) => {
    ...
    const createSelectField = <T extends { [index: string]: any }>({ def, widgetState }: { def: SelectFieldDescription<T>; widgetState: ReturnType<typeof raw> }) => (
      <IonItem key={def.name.toString()}>
        <IonLabel position={def.position} color={def.color}>
          {def.label}
        </IonLabel>
        <IonSelect onIonChange={widgetState.onChange} value={widgetState.value}>
          {def.options.map(({ key, value }) => (
    ...
    const validator = (field: FieldDescriptionType<T>) => (value: string, values: StateValues<T>, event?: React.ChangeEvent<InputElement> | React.FocusEvent<InputElement>) =>
      field.validators && field.validators.reduce((acc, validator) => (acc ? acc : validator(value, values, event)), undefined as undefined | string);

    const validateForm = () => fields.map(f => f.validators && { name: f.label, error: validator(f)(formState.values[f.name], formState.values) }).filter(result => result && result.error);

    const formIsValid = () => Object.keys(formState.validity).length > 0 && Object.values(formState.validity).reduce((acc, value) => acc && value, true);
    ...
    return (
      <div>
        ...
        <IonToast isOpen={errorMessage ? errorMessage.length > 0 : false} onDidDismiss={() => dispatch(createActions(name).error(''))} message={errorMessage} duration={5000} color="danger" />
