...
export interface TextFieldDescription<T extends { [index: string]: any }>
  extends BaseField<T> {
  type: TextFieldTypes;
}

// Properties specific to date fields
export interface DateFieldDescription<T extends { [index: string]: any }>
  extends BaseField<T> {
  type: "date";
  min?: string;
  max?: string;
...
export interface SelectFieldDescription<T extends { [index: string]: any }>
  extends BaseField<T> {
  type: "select";
...
const createActions = (name: string) => ({
  loading: createCustomAction<
    string,
    [boolean],
    { name: string; isLoading: boolean }
  >("form/loading", (isLoading) => ({ name, isLoading })),
  error: createCustomAction<
    string,
    [string],
    { name: string; errorMessage: string }
  >("form/error", (errorMessage: string) => ({ name, errorMessage })),
});
...
export const formBuilderReducer = createReducer<FormBuilderState, FormActions>(
  ...
  .handleType("form/error", (state, action) => ({
    ...state,
    [action.name]: { ...state[action.name], errorMessage: action.errorMessage },
  }))
...
export const BuildForm = <T extends { [index: string]: any }>({
  ...
  Form: ({ handleSubmit, initialState }: FormProps<T>) => {
    ...
    const createTextField = <T extends { [index: string]: any }>({
      ...
      <IonItem key={def.name.toString()}>
        ...
        <IonInput
          {...widgetState}
          {...{ type: def.type }}
          onIonChange={widgetState.onChange}
          ref={myRef}
        />
        {showError(def.name)}
      </IonItem>
    ...
    const createDateField = <T extends { [index: string]: any }>({
      def,
      widgetState,
    }: {
      ...
      const dateProps = Object.entries(def).reduce(
        (acc, [key, value]) =>
          value && datePropsNames.includes(key)
            ? { ...acc, [key]: value }
            : acc,
        {}
      );
    ...
    const createSelectField = <T extends { [index: string]: any }>({
      def,
      widgetState,
    }: {
    ...
    const fieldCreator = (
      def: FieldDescriptionType<T>,
      widgetState: ReturnType<typeof raw>
    ) => {
      switch (def.type) {
        case "date":
          return createDateField({
            def: def as DateFieldDescription<T>,
            widgetState,
          });
        case "select":
          return createSelectField({
            def: def as SelectFieldDescription<T>,
            widgetState,
          });
        default:
          return createTextField({
            def: def as TextFieldDescription<T>,
            widgetState,
          });
      }
    };

    const valueExtractor = (e: React.ChangeEvent<HTMLInputElement>) =>
      e.target.value;
    const validator =
      (field: FieldDescriptionType<T>) =>
      (
        value: string,
        values: StateValues<T>,
        ...
        field.validators.reduce(
          (acc, validator) => (acc ? acc : validator(value, values, event)),
    ...
    const formIsValid = () =>
      Object.keys(formState.validity).length > 0 &&
      Object.values(formState.validity).reduce(
        (acc, value) => acc && value,
        true
      );
        ...
        <form
          onSubmit={(e) => {
            ...
            if (errors.length > 0) {
              ...
              dispatch(
                error(
                  `<p>Form Errors:<p><ul>${errors.map(
                    (e) => e && "<li>" + e.name + ": " + e.error + "</li>"
                  )}</ul>`
                )
              );
            } else {
