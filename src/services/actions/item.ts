    ...
    return aquariumClient.coralPUT("SchiScho", coralId, coral)
      .then(coralResponse => {
        if (coralResponse.hasError) {
          return dispatch(editCoralActions.failure(new Error(`An error occurred: ${coralResponse.errorMessages}`)))
        }
    ...
    return aquariumClient.animalPUT("SchiScho", animalId, animal)
      .then(animalResponse => {
        if (animalResponse.hasError) {
          return dispatch(editAnimalActions.failure(new Error(`An error occurred: ${animalResponse.errorMessages}`)))
        }
...
export const addCoralAction = (coral: Coral): ThunkAction<Promise<AddCoralResult>, RootState, null, AnyAction> =>
  (dispatch, getState) => {
    ...
    return aquariumClient.coralPOST(aquarium, coralRequest)
      .then(coralResponse => {
        if (coralResponse.hasError) {
          return dispatch(addCoralActions.failure(new Error(`An error occurred: ${coralResponse.errorMessages}`)))
        }
...
export const addAnimalAction = (animal: Animal): ThunkAction<Promise<AddAnimalResult>, RootState, null, AnyAction> =>
  (dispatch, getState) => {
    ...
    return aquariumClient.animalPOST(aquarium, animalRequest)
      .then(animalResponse => {
        if (animalResponse.hasError) {
          return dispatch(addAnimalActions.failure(new Error(`An error occurred: ${animalResponse.errorMessages}`)))
        }
